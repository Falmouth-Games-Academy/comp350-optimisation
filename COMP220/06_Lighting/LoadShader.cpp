...
bool LoadShader::compileShader(GLuint shaderId, const std::string& shaderFileName)
{
	...
	{
		error.custom(shaderFileName.c_str(), "File not found");
		return false;
	}
	...
	if (infoLogLength > 1)
	{
		...
		error.custom(errorMessage.data(), shaderFileName.c_str());
	}

	return (result != GL_FALSE);
}

GLuint LoadShader::loadShaders(const std::string& vertex_file_path, const std::string& fragment_file_path)
{
	// Create the shaders
	...
	if (infoLogLength > 1) {
		...
		error.custom(errorMessage.data(), "glLinkProgram error");
	}
